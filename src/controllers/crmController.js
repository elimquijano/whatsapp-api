...
import { buildMediaMessage, MAX_MEDIA_BYTES, sendTextMessage } from "../services/messageService.js";
...
export const listContacts = async (req, res) => {
  try {
    ...
    const contacts = await CrmContact.findAll({ where, order: [["priority", "DESC"], ["lastMessageAt", "DESC"]], limit: 500 });
...
export const getContactMessages = async (req, res) => {
  try {
    if (!await professionalGuard(req, res)) return;
    const contact = await CrmContact.findByPk(req.params.contactId, { include: [{ model: WhatsAppSession, as: "whatsappSession", where: { userId: req.user.id } }] });
    ...
    res.json({ success: true, contact, messages });
  } catch (error) { res.status(500).json({ success: false, error: error.message }); }
};
...
export const sendManualMessage = async (req, res) => {
  try {
    ...
    const jid = contact.contactJid || `${contact.phone}@s.whatsapp.net`;
...
export const saveImportSource = async (req, res) => {
  try {
    ...
    res.json({ success: true, source: { ...source.toJSON(), authValue: "", hasAuthValue: Boolean(source.authValue) } });
...
export const runImport = async (req, res) => {
  try {
    if (!await professionalGuard(req, res)) return;
    const source = await CrmImportSource.findByPk(req.params.sourceId, { include: [{ model: WhatsAppSession, as: "whatsappSession", where: { userId: req.user.id } }] });
    ...
    for (const record of records) {
      const phone = String(getPath(record, source.fieldMapping.phone) || "").replace(/\D/g, "");
      if (!phone) continue;
      ...
      if (!created) {
        const changes = {
          contactJid: contact.contactJid || `${phone}@s.whatsapp.net`,
    ...
    res.json({ success: true, imported, totalReceived: records.length });
  } catch (error) { res.status(400).json({ success: false, error: error.message }); }
};
...
export const listCampaigns = async (req, res) => {
  try {
    if (!await professionalGuard(req, res)) return;
    const sessions = await WhatsAppSession.findAll({ where: { userId: req.user.id }, attributes: ["id"] });
    const storedCampaigns = await CrmCampaign.findAll({
      where: { whatsappSessionId: sessions.map((item) => item.id) },
...
export const createCampaign = async (req, res) => {
  ...
  try {
    ...
    const mediaPayload = messageType === "text" ? null : (req.body.mediaPayload || null);
    ...
    const where = { whatsappSessionId: session.id };
    if (req.body.filters?.statuses?.length) where.status = { [Op.in]: req.body.filters.statuses };
    if (req.body.contactIds?.length) where.id = { [Op.in]: req.body.contactIds };
...
export const runCampaign = async (req, res) => {
  try {
    if (!await professionalGuard(req, res)) return;
    const campaign = await CrmCampaign.findByPk(req.params.campaignId, { include: [{ model: WhatsAppSession, as: "whatsappSession", where: { userId: req.user.id } }] });
