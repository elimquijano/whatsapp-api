      ...
      <Stack spacing={1.5} sx={{ p: 1.75, minHeight: 0, overflow: 'auto' }}>
        ...
        {node.type === 'http_request' && <>
          ...
          <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1}>
            ...
            <TextField size="small" fullWidth type="number" label="Respuesta máxima (MB)" value={config.maxResponseMb ?? 25} onChange={(event) => updateConfig({ maxResponseMb: Number(event.target.value) })} inputProps={{ min: 1, max: 100, step: 1 }} helperText="Evita reinicios por memoria; no limita la cantidad de registros." />
          </Stack>
