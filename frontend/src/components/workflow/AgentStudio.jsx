    ...
    <Box sx={{ flex: 1, minHeight: 0, display: 'grid', gridTemplateColumns: { xs: '1fr', lg: '230px minmax(360px, 1fr) 230px' }, overflow: { xs: 'auto', lg: 'hidden' } }}>
      ...
      <Stack spacing={1.5} sx={{ p: 1.75, minHeight: 0, overflow: 'auto' }}>
        ...
        {node.type === 'http_request' && <>
          ...
          <TextField size="small" fullWidth type="number" label="Tiempo máximo (segundos)" value={Math.round((config.timeoutMs ?? 30000) / 1000)} onChange={(event) => updateConfig({ timeoutMs: Number(event.target.value) * 1000 })} inputProps={{ min: 1, max: 120, step: 1 }} helperText="Hasta 120 segundos para APIs lentas; la cantidad de registros no se limita." />
          ...
          <Alert severity="info" icon={false}>Ejemplo: <code>{'{"id":"id","nombre":"datos.nombre","precio":"datos.precio"}'}</code>. Todos los registros continúan completos hacia los nodos siguientes; la interfaz solo dibuja 5 elementos para mantenerse rápida.</Alert>
        </>}
        {node.type === 'script' && <>
